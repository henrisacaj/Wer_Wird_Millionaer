...
public class Game {
  ...
  void init(List<Question> allQuestions) {
    ...
    allQuestions.add(new Question("Mit der angeheirateten Verwandschaft ist man ...?", 1,
        Arrays.asList(new String[] { "verschwaegert", "verbruedert", "vernefft", "vernichtet" }),
        0));
    allQuestions
        .add(new Question("Bei einem gut gelaunten Zombie handelt es sich sozusagen um einen...?",
            1, Arrays.asList(new String[] { "kah Freitag", "kristi Himmelfahrt", "fingst Montag",
                "frohen Leichnam" }),
            3));
    allQuestions.add(new Question("Was gehoert fuer manche zu ihrem Taetigkeitsfeld?", 1,
        Arrays.asList(new String[] { "veranwalten", "verbaeckern", "verarzten", "verfoerstern" }),
        2));
    allQuestions.add(new Question("Ein Zeuge hatte genau beobachtet, was der ...?", 1,
        Arrays.asList(new String[] { "Bankueber fall", "Handtaschen raub", "Ladendieb stahl",
            "Trick betrug" }),
        2));
    allQuestions.add(new Question(
        "Ist das alte ranzig geworden, ist es Zeit, dass ich vor dem Anmachen des Salats das ...?",
        1, Arrays.asList(
            new String[] { "Licht test", "Motor waesche", "Reifen service", "Oel wechsel" }),
        3));

    // 100€ Fragen
    allQuestions.add(new Question("Wer wird in der Polizeikontrolle zur Kasse gebeten?", 2,
        Arrays.asList(
            new String[] { "Airbagnoergler", "Lenkerstaenkerer", "Kuppelquengler", "Gurtmuffel" }),
        3));
    allQuestions.add(new Question(
        "Verzweifelt man an einer Sache, ist das einer Redensart zufolge zum...?", 2,
        Arrays.asList(
            new String[] { "Schafescheren", "Maeusemelken", "Pferdemaesten", "Gaensekastrieren" }),
        1));
    allQuestions.add(new Question(
        "Suchte die zweifache Eiskunstlauf-Olympiasiegerin Witt das Weite, dann...?", 2,
        Arrays.asList(new String[] { "taep Pich", "vohr Leger", "leu Fer", "floh Kati" }), 3));
    allQuestions.add(new Question("Wem es an Kaufkraft mangelt, der ist redensartlich ...?", 2,
        Arrays.asList(new String[] { "kurz in Boerse", "knapp bei Kasse", "schmal auf Konto",
            "blank trotz Scheck" }),
        1));
    ...
    // 200€ Fragen
    allQuestions.add(new Question("Wie nennt man eine leicht zusammenklappbare Schlafstaette?", 3,
        Arrays.asList(new String[] { "Ackerliege", "Feldbett", "Wiesenkoje", "Weidenpritsche" }),
        1));
    allQuestions.add(
        new Question("Twitter ist ein...?", 3, Arrays.asList(new String[] { "Mikroblogging-Dienst",
            "Maxijogging-Plan", "Megabugging-Werkzeug", "Mikrobugging-Dienst" }), 0));
    allQuestions.add(new Question(
        "Sieht der Trainer vom Einsatz eines angeschlagenen Spielers zurzeit lieber ab, ist dieser sozusagen nicht in...?",
        3, Arrays.asList(
            new String[] { "Bratpfanne", "Suppenkessel", "Schnellkochtopf", "Auflaufform" }),
        3));
    allQuestions.add(new Question("Welche Haut ist kein Bestandeil des menschlichen Auges?", 3,
        Arrays.asList(new String[] { "Hornhaut", "Netzhaut", "Regenbogenhaut", "Gaensehaut" }), 3));
    ...
    allQuestions.add(new Question(
        ...
        Arrays.asList(new String[] { "Rein-Hard", "Wil-Helm", "Man-Fred", "Wolf-Gang" }), 3));
    allQuestions.add(new Question("In Deutschland gibt es 17 ...", 4,
        Arrays.asList(
            new String[] { "Anwaltsstuben", "Managerbuden", "Aertzekammern", "Pilotenkabuffs" }),
        2));
    allQuestions.add(new Question("Was gibt es in vielen Skigebieten?", 4,
        Arrays.asList(
            new String[] { "Trottelberg", "Deppenschanze", "Idiotenhuegel", "Schwachmatenpiste" }),
        2));
    allQuestions.add(new Question("Kaum hatte das Milchgeschaeft geoeffnet, wollte schon der ...?",
        4,
        Arrays.asList(
            new String[] { "Absolute Spitze", "Voll der Hammer", "Erste Sahne", "Grosse Klasse" }),
        2));
    ...
    // 500€ Fragen
    allQuestions.add(new Question(
        "Wer heisst im amerikanischen Original der \"Sesamstrasse\" \"Count Von Count\"?", 5,
        Arrays.asList(
            new String[] { "Fuerst Buchstabe", "Herzog Ziffer", "Graf Zahl", "Prinz Zeichen" }),
        2));
    allQuestions.add(new Question("Was machte 2006 Klinsi mit Woerns?", 5,
        Arrays.asList(new String[] { "zurueckrudern", "einschiffen", "ausbooten", "auftakeln" }),
        2));
    allQuestions.add(new Question("In der Vorweihnachtszeit beschenkt man sich anonym beim ...?", 5,
        Arrays.asList(new String[] { "Zwergeln", "Gnomeln", "Maedchen Rosemarie", "Superweib" }),
        2));
    allQuestions.add(new Question(
        "Die Geschichte des Mordfalls Nitribitt wurde 1958 erstmals verfilmt als \"Das ...\"?", 5,
        Arrays.asList(
            new String[] { "Schwarzwaldmaedel", "Rosen-Resli", "Maedchen Rosemarie", "Superweib" }),
        2));
    allQuestions.add(new Question(
    ...
    // 1000€ Fragen
    allQuestions.add(
        new Question("Johann Wolfgang von Goethes Autobiographie kennt man unter dem Titel ...?", 6,
    ...
    allQuestions.add(new Question(
        ...
        6, Arrays.asList(new String[] { "Hannover", "Duesseldorf", "Stuttgart", "Potsdam" }), 1));
    allQuestions.add(new Question(
        "Was war bereits seit Mai 1969 ein beliebtes Zahlungsmittel im europaeischen Raum?", 6,
        Arrays.asList(new String[] { "Euronoten", "Eurocheques", "Euroscheine", "Euromuenzen" }),
        1));
    allQuestions.add(new Question(
        "Wofuer ist bereits in der Planungsphase die Statik von entscheidender Bedeutung?", 6,
        Arrays.asList(new String[] { "Blinken", "Schalten", "Kuppeln", "Abwuergen" }), 2));
    allQuestions.add(new Question(
    ...
    // 2000€ Fragen
    allQuestions.add(new Question("Wer weiss wohl am ehesten, was mit \"Jumpsuit\" gemeint ist?", 7,
        Arrays.asList(new String[] { "Modekenner", "Boersen-Spekulanten", "Stabhochspringer",
            "Autoverkaeufer" }),
        0));
    ...
    allQuestions.add(new Question(
        ...
        7, Arrays.asList(
            new String[] { "Rueckbank", "Scheibenwischer", "Tankanzeige", "Schneeketten" }),
        1));
    allQuestions.add(new Question(
        "Worum geht es ueblicherweise, wenn von einem \"Schwanenhalsrahmen\" die Rede ist?", 7,
        Arrays.asList(new String[] { "Hornbrillen", "Oelgemaelde", "Tankanzeige", "Fahrraeder" }),
        3));
    allQuestions.add(new Question(
        "Wer gewann 2015 als erster Deutscher seit zwoelf Jahren wieder ein Springen der Vierschanzentournee?",
        7, Arrays.asList(new String[] { "Montag", "Dienstag", "Donnerstag", "Freitag" }), 3));
    allQuestions.add(new Question(
        "Was mancher selbst im nuechternen Zustand nicht hinbekommt: Korrekt schreibt sich der beliebte Cocktail ...?",
        7, Arrays.asList(new String[] { "Caipirinja", "Cajpirinha", "Caipirinha", "Caijpiriña" }),
        2));

    // 4000€ Fragen
    allQuestions.add(new Question(
        "Von welchem nur 321 Meter hohen Berg wird behauptet, der meistbestiegene Europas zu sein?",
        8, Arrays.asList(new String[] { "Brocken", "Drachenfels", "Feldberg", "Zugspitze" }), 1));
    allQuestions
        .add(new Question("Wessen Ansprueche sollen hierzulande gesetzlich neu geregelt werden?", 8,
            Arrays.asList(new String[] { "Muenzmuetter", "Scheinvaeter", "Bargeldeltern",
                "Kerditkartenfamilien" }),
            1));
    allQuestions.add(new Question(
        "Woraus besteht in der Regel eine Entourage?", 8, Arrays.asList(new String[] {
            "Baguette & Rotwein", "Mascara & Lidschatten", "Freunde & Bekannte", "Sofa & Sessel" }),
        2));
    allQuestions.add(new Question(
        "Den Andenbaeren, die einzigen suedamerikanische Baerenart, kennt man auch unter der Bezeichnung ...?",
        8,
        Arrays.asList(new String[] { "Toupetbaer", "Brillenbaer", "Gebissbaer", "Hoergeraetbaer" }),
        1));
    allQuestions.add(new Question(
        "Wobei wird vor einem sogenannten Rebound-Effekt gewarnt, der nicht selten zu einer Abhaengigkeit fuehrt?",
        8, Arrays.asList(new String[] { "Haarspray", "Nasenspray", "Deospray", "Pfefferspray" }),
        1));
    ...
    allQuestions.add(new Question(
        ...
        Arrays.asList(
            new String[] { "Rialtobruecke", "Pont Neuf", "Golden Gate Bridge", "Oeresundbruecke" }),
        0));
    allQuestions
        .add(new Question("Wobei besteht die oberste Schicht oft aus dem sogenannten Fondant?", 9,
            Arrays.asList(new String[] { "Kaltschaummatratze", "Komposthaufen", "Festtagstorte",
                "Strassenbelag" }),
            2));
    allQuestions.add(
        new Question("Was haben die Hollywood-Stars Gosling, Reynolds und Phillippe gemeinsam?", 9,
            Arrays.asList(new String[] { "Vorname Ryan", "Ex-Frau Megan Fox", "Geburtsjahr 1978",
                "irische Staatsbuergerschaft" }),
            0));
    allQuestions.add(new Question(
        "Wie heisst der aktuelle Abenteuer-Kinofilm mit Jake Gyllenhaal und Josh Brolin in den Hauptrollen?",
        9, Arrays.asList(new String[] { "Mississippi", "Everest", "Sahara", "Lueneburger Heide" }),
        1));
    allQuestions.add(new Question(
        "Die Darstellung welcher Figur wurde schon zweimal mit einem Schauspiel-Oscar praemiert?",
        9, Arrays.asList(
            new String[] { "Dr. Hannibal Lecte", "Forrest Gump", "Joker", "Truman Capote" }),
        2));

    // 16000€ Fragen
    allQuestions.add(
        new Question("Was bedeutet der Name der italienischen Nachspeise \"Tiramisu\" woertlich?",
            10, Arrays.asList(new String[] { "suess und koestlich", "zieh mich hoch",
    ...
    allQuestions.add(new Question(
        ...
        10, Arrays.asList(new String[] { "Basketball", "Volleyball", "Handball", "Tennis" }), 2));
    ...
    allQuestions.add(new Question("Wo befinden sich einige der hoechsten Alpengipfel?", 11,
        ...
        0));
    allQuestions.add(new Question(
        "Wofuer stehen die Buchstaben im Namen der britischen Automarke MG?", 11,
        Arrays.asList(
            new String[] { "Moto Guzzi", "Motor Gear", "Morris Garages", "Manchester Gasoline" }),
        2));
    allQuestions.add(new Question(
    ...
    allQuestions.add(new Question(
        ...
        12, Arrays.asList(new String[] { "Rhodesien", "Jugoslawien", "Ceylon", "Tibet" }), 1));
    allQuestions.add(new Question(
        "Welcher Begriff spielt im Zusammenhang mit der Rechenleistung von Computern eine Rolle?",
        12, Arrays.asList(new String[] { "Busbreiten", "Radhoehen", "Autotiefen", "Zuglaengen" }),
        0));
    allQuestions.add(new Question("Wer erfand die Raufasertapete?", 12,
        Arrays.asList(
            new String[] { "Gustav Chemnitz", "Hugo Erfurt", "Alfons Zwickau", "Emil Schwerin" }),
        1));
    allQuestions.add(new Question(
        "Wie lautet der vollstaendige Vorname von Expraesident und Nobeltraeger Jimmy Carter?", 12,
        Arrays.asList(new String[] { "Jim Bob", "John Ross", "James Earl", "Joseph Conrad" }), 2));
    allQuestions.add(new Question("Worauf deutet das “V“ bei einem V-Motor hin?", 12, Arrays
        .asList(new String[] { "Bauform", "Heckantrieb", "Kraftstoffart", "Zylinder-Anzahl" }), 0));

    // 125000€ Fragen
    allQuestions.add(new Question("Was war hierzulande bis in die 1950er noch gang und gebe?", 13,
        Arrays.asList(new String[] { "Beamtinnenkommunion", "Krankenschwesternkollete",
            "Lehrerinnenzoelibat", "Sekretaerinnenbeichte" }),
        2));
    allQuestions.add(new Question("Welche Pflanze gibt es nicht?", 13,
        Arrays.asList(new String[] { "Pimpernuss", "Zaubernuss", "Wassernuss", "Buchnuss" }), 3));
    allQuestions.add(new Question(
    ...
    allQuestions.add(new Question("Wo liegt Franz-Josef-Land?", 13,
        Arrays.asList(
    ...
    allQuestions.add(new Question(
        ...
        14, Arrays.asList(new String[] { "Bambus", "Ginkgo", "Rhododendron", "Eukalyptus" }), 0));
    allQuestions.add(new Question(
        ...
        0));
    allQuestions.add(new Question("Wie hiess das erste deutsche Frachtschiff mit Atomantrieb?", 14,
        Arrays.asList(new String[] { "Albert Einstein", "Otto Hahn", "Wilhelm Conrad Roentgen",
            "Georg Ohm" }),
        1));
    allQuestions.add(new Question("Welche deutschen Namen traegt das Insekt Paraponera clavata?",
        14, Arrays.asList(new String[] { "60-Sekunden-Kaefer", "5-Minuten-Wanze",
            "24-Stunden-Ameise", "9 1/2-Wochen-Biene" }),
        2));
    allQuestions.add(new Question(
        "Bei welcher Taetigkeit benutzte man der Wortherkunft nach urspruenglich eine Rakete?", 14,
        Arrays.asList(new String[] { "Backen", "Pfluegen", "Spinnen", "Toepfern" }), 2));
    ...
    allQuestions.add(new Question(
        ...
        15, Arrays.asList(
            new String[] { "1.011,2 km/h", "1.225,4 km/h", "1.357,6 km/h", "1.483,8 km/h" }),
        2));
    allQuestions.add(new Question("Mit welcher Verletzung beschaeftigt sich der Orthopaede?", 15,
        Arrays.asList(
            new String[] { "Meysel-Pyramide", "Volkmann-Dreieck", "Hancke-Trapez", "Mira-Kegel" }),
        1));
    allQuestions.add(new Question(
        "Welche Tiere werden vor allem in Nordamerika vom meist toedlich verlaufenden White-Nose-Syndrom befallen?",
        15, Arrays.asList(
            new String[] { "Fledermaeuse", "Maeusebussarde", "Waldmeisen", "Ameisenbaeren" }),
        3));
    allQuestions.add(new Question("Welche Frucht nennen die Italianer “Arancio Dolce“ ?", 15,
        Arrays.asList(new String[] { "Mandarine", "Pampelmuse", "Zitrone", "Apfelsine" }), 3));
  }
  ...
  void start() {
    ...
    for (int currentDifficulty = 1; currentDifficulty < 15; currentDifficulty++) {
      int[] cashPrizes = { 50, 100, 200, 300, 500, 1000, 2000, 4000, 8000, 16000, 32000, 64000,
      ...
      if (userAnswer == currentQuestion.getCorrectAnswer()) {
        if (currentDifficulty == 15)
          System.out.println("Herzlichen Glueckwunsch! Du bist jetzt ein virtueller Millionaer.");
        else
        ...
        if (currentDifficulty <= 5) {
          ...
          System.out.println(
              "Deine Antwort war falsch! Die gewonnene Summe liegt bei " + cashPrizes[4] + "€!");
          break;
        } else {
          System.out.println(
              "Deine Antwort war falsch! Die gewonnene Summe liegt bei " + cashPrizes[9] + "€!");
          break;
        }
      }
    }
